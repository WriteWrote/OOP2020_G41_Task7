...
public class SimpleHashMultiSet<T> implements IMultiSet<T> {
    ...
    public SimpleHashMultiSet(int def_capacity) {
        ...
        LOAD_FACTOR = .75;
        table = (Node<T, Integer>[]) new Node[DEF_CAPACITY];
    }
    ...
    public boolean add(T element, int occurrences) {
        ...
        Node<T, Integer> newElement = new Node<>(hash, element, occurrences, null);
        ...
        if (table[index] == null) {
            ...
            while (oldElement.getNext() != null) {
                if (oldElement.equals(newElement)) {
                    oldElement.setValue(oldElement.getValue() + occurrences);
                }
            ...
            while (currElement != null) {
                if (hash == currElement.getHash() && Objects.equals(element, currElement.getKey())) {
                    if (n == 0) {
                        ...
                        if (buff.getNext().getValue() == 1)
                            buff.setNext(buff.getNext().getNext());
                        else buff.getNext().setValue(buff.getNext().getValue() - 1);
                    }
                    --numberOfElements;
                    modified = true;
